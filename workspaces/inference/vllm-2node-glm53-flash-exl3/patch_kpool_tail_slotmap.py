...
VENDORED, and that is deliberate. Every other patch this workspace relies on
is already applied inside the overlay image, so `up.sh` re-runs them only as a
cheap repair and skips the ones a rebuilt image dropped. This one is different
on both counts: it is a CORRECTNESS fix - the unpatched kernel writes through a
garbage block id and most overruns land inside the shared pool, corrupting
another layer's indexer rather than crashing - and it postdates the published
`:exl3` tag, so `[ -f /opt/glm53/... ]` would silently find nothing and skip it.
A silent skip is exactly the outcome the fix exists to prevent, so the file is
carried here, bind-mounted into both ranks, and running it is NOT optional.

Upstream: MiaAI-Lab/GLM-5.3-Flash-EXL3-2x-DGX-Sparks, overlay/, MIT. Copied
verbatim apart from this note - see docs/decisions.md#glm53-second-pass for why
this repo vendors a third-party patch when it vendors nothing else.
"""
